...
erlc -Wall -DTEST -o "${test_dir}" src/avm_cbor.erl src/avm_cbor_partial.erl
...
erl -noshell -pa "${test_dir}" -eval \
    'case eunit:test([avm_cbor_coverage_tests, avm_cbor_deterministic_decode_tests, avm_cbor_security_tests, avm_cbor_property_tests], [verbose]) of ok -> halt(0); _ -> halt(1) end.'
